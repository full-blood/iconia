...
(
    ...
    (
		...
		button btn_manual_s2 "MANUAL RENAME" pos:[275,400] width:120 height:60 visible:false
        ...
        (
            local allCtrls = #(
                ...
                lst_log_s2, lbl_prev_title_s2, edt_before_s2, lbl_arrow_s2, edt_after_s2, btn_main_s2, btn_manual_s2, btn_confirm_s2,
...
                for c in #(lst_log_s2, lbl_prev_title_s2, edt_before_s2, lbl_arrow_s2, edt_after_s2) do c.visible = true
                if step2_state == 2 then (
                    btn_manual_s2.visible = true
                    btn_confirm_s2.visible = true
                ) else (
                    btn_main_s2.visible = true 
                )
            )
...
            )
            
            -- NOUVEAU NETTOYAGE D'ANNÉE EN LIGNE (Sans créer de sous-fonction)
            local beforeYearCleanup = s
            local yearFound = true
            while yearFound do (
                yearFound = false
                for i = 1 to (s.count - 3) do (
                    local p1 = substring s i 1
                    local p2 = substring s (i+1) 1
                    if (p1 == "1" and p2 == "9") or (p1 == "2" and p2 == "0") then (
                        local p3 = substring s (i+2) 1
                        local p4 = substring s (i+3) 1
                        if (p3 >= "0" and p3 <= "9") and (p4 >= "0" and p4 <= "9") then (
                            local beforeIdx = i - 1
                            local afterIdx = i + 4
                            local cBefore = if beforeIdx >= 1 then substring s beforeIdx 1 else ""
                            local cAfter = if afterIdx <= s.count then substring s afterIdx 1 else ""
                            
                            local isYearWrapBefore = (cBefore == "" or cBefore == "_" or cBefore == "-" or cBefore == " " or cBefore == "[" or cBefore == "(" or cBefore == "{")
                            local isYearWrapAfter = (cAfter == "" or cAfter == "_" or cAfter == "-" or cAfter == " " or cAfter == "]" or cAfter == ")" or cAfter == "}")
                            
                            if isYearWrapBefore and isYearWrapAfter then (
                                local startCut = i
                                local endCut = i + 3
                                if cBefore == "[" or cBefore == "(" or cBefore == "{" then startCut -= 1
                                if cAfter == "]" or cAfter == ")" or cAfter == "}" then endCut += 1
                                
                                s = (substring s 1 (startCut - 1)) + (substring s (endCut + 1) -1)
                                yearFound = true
                                exit
                            )
                        )
...
                )
            )
            if s != beforeYearCleanup then hasYear = true
            
...
            btn_confirm_s2.visible = false; btn_manual_s2.visible = false; btn_main_s2.visible = true; btn_main_s2.enabled = false; btn_main_s2.text = "RENAME\nDONE ✔"; step2_state = 3 
            switchStep 3
        )
...
            local hasCoronaFlag = result[2]
            local hasYearFlag = result[3]
            
...
            if hasYearFlag then append logLines "✓  Year detected → will be removed"
            
...
            )
            
            append logLines ""; append logLines "Review the rename below, then confirm."
            lst_log_s2.items = logLines
            edt_before_s2.text = rawBase
            edt_after_s2.text = cleanedName
            edt_after_s2.readOnly = true
            btn_manual_s2.enabled = true
            
            btn_main_s2.visible = false
            btn_manual_s2.visible = true
            btn_confirm_s2.visible = true
            step2_state = 2
        )
		...
		-- BOUTON MANUAL RENAME
        on btn_manual_s2 pressed do (
            edt_after_s2.readOnly = false
            setFocus edt_after_s2
            btn_manual_s2.enabled = false
        )

		-- BOUTON CONFIRM RENAME
